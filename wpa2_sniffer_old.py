...
class sniffer():
	...
	def decrypt(self, packet):
		...
		encrypted_mess = hexlify(bytes(packet[Raw]))
...
#		unencrypted_mess = self.filter_packets(unencrypted_mess)
	...
	def filtersniff(self, packet):
		try:
			...
			if dst == self.mac or src == self.mac:
	...
	def make_cap(self):
		print(self.dfilepath + "/AirodumpWPA2.py {} {}".format(self.mac, self.dfilepath))

...
def get_col(matrix, col_nb):
	col = []

	for i in range(len(matrix)):
		col.append(matrix[i][col_nb])
...
def s_sub(key_col):
	...
	for elem in key_col:
		...
		if len(elem) < 2:
			elem = '0' + elem
		if elem[0].isalpha():
			first_coor = ord(elem[0]) - ord('a') + 10
		else:
			first_coor = elem[0]
		if elem[1].isalpha():
			sec_coor = ord(elem[1]) - ord('a') + 10
		else:
			sec_coor = elem[1]

		new_col.append(sbox[int(first_coor)][int(sec_coor)])
...
def get_round_key(key):
	...
	first_col = xor(xor(get_col(key, 0), s_sub_rword), get_col(rcon, 0))
	remove_col(rcon, 0)
	round_key = make_empty_matrix(len(key))
	add_col(round_key, first_col)

	for i in range(1, len(key[0])):
		add_col(round_key, xor(get_col(key, i), get_col(round_key, i - 1)))

	return round_key


def make_key_streams(key, rounds=10):
	key_streams = dict()
